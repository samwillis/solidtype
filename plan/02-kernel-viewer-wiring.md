...
const supportsOffscreen =
  typeof OffscreenCanvas !== "undefined" && canvas.transferControlToOffscreen !== undefined;
...
features.observeDeep((events) => {
  ...
  const firstChangedIndex = features
    .toArray()
    .findIndex((f) => changedFeatureIds.has(f.getAttribute("id")));
...
describe("YjsWorkerSync", () => {
  ...
  test("sends updates when document changes", async () => {
    ...
    await waitFor(() => {
      expect(worker.messages).toContainEqual(expect.objectContaining({ type: "update" }));
    });
  });
