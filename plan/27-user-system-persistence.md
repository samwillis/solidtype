...
export function createDocumentSync(documentId: string, doc: Y.Doc) {
  ...
  return {
    ...
    awarenessProvider,
    connect: async () => {
      await docProvider.connect();
      await awarenessProvider.connect();
    },
    disconnect: () => {
...
There's a planned `y-durable-streams` package (see [PR #81](https://github.com/durable-streams/durable-streams/pull/81)) that provides a Yjs provider for Durable Streams. Until it's merged and released, we vendor the provider code into our codebase.

**Vendored location:** `packages/app/src/lib/vendor/y-durable-streams/`

```typescript
// packages/app/src/lib/vendor/y-durable-streams/index.ts
...
The vendored provider wraps `@durable-streams/client` with Yjs-specific logic:
...
When `@durable-streams/y-durable-streams` (or similar) is released:
