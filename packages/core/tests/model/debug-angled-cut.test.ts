...
import { subtract } from "../../src/model/boolean.js";
import { planarBoolean } from "../../src/boolean/planar/planarBoolean.js";
import { tessellateBody } from "../../src/mesh/tessellateBody.js";
import { extrude } from "../../src/model/extrude.js";
import { createPolygonProfile } from "../../src/model/sketchProfile.js";
import { YZ_PLANE, XY_PLANE } from "../../src/model/planes.js";
...
describe("angled cut debug", () => {
  it("reproduces the app angled cut issue", () => {
    ...
    for (const shellId of baseShells) {
      ...
      for (const faceId of faces) {
        ...
        if (surface.kind === "plane") {
          console.log(
            `  Face ${faceId}: normal=[${surface.normal[0].toFixed(2)},${surface.normal[1].toFixed(2)},${surface.normal[2].toFixed(2)}]`
          );
        }
      }
    }
    ...
    for (const shellId of toolShells) {
      ...
      for (const faceId of faces) {
        ...
        if (surface.kind === "plane") {
          ...
          console.log(
            `  Face ${faceId}: normal=[${n[0].toFixed(3)},${n[1].toFixed(3)},${n[2].toFixed(3)}]${isTilted ? " (TILTED)" : ""}`
          );
        }
      }
    }
    ...
    console.log(
      `Subtract result: success=${cutResult.success}, warnings=${JSON.stringify(cutResult.warnings)}`
    );
    ...
    for (const shellId of resultShells) {
      ...
      for (const faceId of faces) {
        ...
        if (surface.kind === "plane") {
          ...
          console.log(
            `  Face ${faceId}: normal=[${n[0].toFixed(3)},${n[1].toFixed(3)},${n[2].toFixed(3)}], loops=${loops.length}, verts=${vertexCount}`
          );
        }
      }
    }
    ...
    console.log(
      `BBox: [${minX.toFixed(1)}, ${minY.toFixed(1)}, ${minZ.toFixed(1)}] to [${maxX.toFixed(1)}, ${maxY.toFixed(1)}, ${maxZ.toFixed(1)}]`
    );
