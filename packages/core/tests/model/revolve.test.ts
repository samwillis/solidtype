...
import {
  ...
  Y_AXIS_REVOLVE,
  Z_AXIS_REVOLVE,
} from "../../src/model/revolve.js";
import {
  createRectangleProfile,
  createPolygonProfile,
  createCircleProfile,
} from "../../src/model/sketchProfile.js";
import { YZ_PLANE, ZX_PLANE, createDatumPlaneFromNormal } from "../../src/model/planes.js";
...
describe(`revolve`, () => {
  ...
  describe(`different axes`, () => {
    ...
    it(`revolves around custom axis`, () => {
      const profile = createRectangleProfile(YZ_PLANE, 1, 1, 2, 0);
      const customAxis = createAxisFromDirection(vec3(0, 0, 0), vec3(0, 1, 1));
      const result = revolve(model, profile, {
        operation: `add`,
        axis: customAxis,
      });
