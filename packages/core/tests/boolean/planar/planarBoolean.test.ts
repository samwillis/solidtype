...
import { dot3 } from "../../../src/num/vec3.js";
...
import { subtract, intersect } from "../../../src/model/boolean.js";
      ...
      for (const faceId of faces) {
        ...
        if (surface.kind === "plane") {
          ...
          console.log(
            ...
            faceId,
            "normal:",
            surface.normal.map((n) => n.toFixed(2)),
            "verts:",
            vertCount
          );
        }
      ...
      for (const faceId of faces) {
        ...
        if (surface.kind === "plane") {
          console.log(
            ...
            faceId,
            "n:",
            surface.normal.map((n) => n.toFixed(1)),
            "verts:",
            verts.map((v) => `(${v.map((c) => c.toFixed(1)).join(",")})`).join(" ")
          );
        }
      }
    ...
    for (const [dx, dy, dz] of seeds) {
      ...
      for (const faceId of faces) {
        ...
        for (const loop of loops) {
          let count = 0;
          for (const _he of model.iterateLoopHalfEdges(loop)) {
            count++;
          }
