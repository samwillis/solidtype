...
import type { Sketch, SketchPointId, SolveResult, SolveOptions } from "./types.js";
import { getSketchPoint, getSketchEntity, DEFAULT_SOLVE_OPTIONS } from "./types.js";
...
function computeConstraintResiduals(state: SolverState, constraint: Constraint): number[] {
  ...
  switch (constraint.kind) {
    case `coincident`: {
      ...
      return [weight * (p1[0] - p2[0]), weight * (p1[1] - p2[1])];
    }
    ...
    case `fixed`: {
      ...
      return [weight * (p[0] - constraint.position[0]), weight * (p[1] - constraint.position[1])];
    }
    ...
    case `concentric`: {
      ...
      if (a1?.kind === `arc` && a2?.kind === `arc`) {
        ...
        return [weight * (c1[0] - c2[0]), weight * (c1[1] - c2[1])];
      }
      return [0, 0];
    }
    ...
    case `symmetric`: {
      ...
      if (line?.kind === `line`) {
        ...
        return [weight * midDist, weight * perpResidual];
      }
      return [0, 0];
    }
    ...
    case `midpoint`: {
      ...
      if (line?.kind === `line`) {
        ...
        return [weight * (p[0] - expectedMid[0]), weight * (p[1] - expectedMid[1])];
      }
      return [0, 0];
    }
    ...
    case `arcArcTangent`: {
      ...
      if (a1?.kind === `arc` && a2?.kind === `arc`) {
        ...
        const expectedDist = constraint.internal ? Math.abs(r1 - r2) : r1 + r2;
 ...
 */
function getLineDir(state: SolverState, startId: SketchPointId, endId: SketchPointId): Vec2 {
 ...
 * Get line length
 */
function getLineLength(state: SolverState, startId: SketchPointId, endId: SketchPointId): number {
  const start = getPointPosition(state, startId);
  const end = getPointPosition(state, endId);
