 ...
 */
export function healModel(model: TopoModel, options: HealingOptions = {}): HealingResult {
  const opts: Required<HealingOptions> = {
    ...
    smallFaceAreaThreshold:
      options.smallFaceAreaThreshold ?? Math.pow(model.ctx.tol.length * 10, 2),
  };
 ...
 */
export function mergeCoincidentVertices(model: TopoModel, tolerance: number): SubOperationResult {
 ...
 */
export function collapseShortEdges(model: TopoModel, threshold: number): SubOperationResult {
  const result: SubOperationResult = { count: 0, actions: [] };
  ...
  for (const { id: edgeId, length } of edgesToCollapse) {
    ...
    const p0 = model.getVertexPosition(vStart);
    const p1 = model.getVertexPosition(vEnd);
    const midpoint: Vec3 = [(p0[0] + p1[0]) / 2, (p0[1] + p1[1]) / 2, (p0[2] + p1[2]) / 2];
 ...
 */
export function removeSmallFaces(model: TopoModel, areaThreshold: number): SubOperationResult {
  const result: SubOperationResult = { count: 0, actions: [] };
...
export function getHealingSummary(model: TopoModel): {
  ...
  return {
    ...
    shortEdges: report.issues.filter((i) => i.kind === `shortEdge` || i.kind === `zeroLengthEdge`)
      .length,
    smallFaces: report.issues.filter((i) => i.kind === `zeroAreaFace` || i.kind === `sliverFace`)
      .length,
    inconsistentShells: report.issues.filter((i) => i.kind === `inconsistentShellOrientation`)
      .length,
  };
}
