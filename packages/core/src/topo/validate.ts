...
function validateEdges(
  ...
  for (let i = 0; i < count; i++) {
    ...
    if (isNullId(vStart)) {
      addIssue(report, 'nullReference', 'error', `Edge ${i} has null start vertex`, { type: 'edge', id: i });
    } else if (model.isVertexDeleted(vStart)) {
      addIssue(report, 'deletedReference', 'error', `Edge ${i} references deleted start vertex ${vStart}`,
        { type: 'edge', id: i }, [{ type: 'vertex', id: vStart }]);
    }
    
    if (isNullId(vEnd)) {
      addIssue(report, 'nullReference', 'error', `Edge ${i} has null end vertex`, { type: 'edge', id: i });
    } else if (model.isVertexDeleted(vEnd)) {
      addIssue(report, 'deletedReference', 'error', `Edge ${i} references deleted end vertex ${vEnd}`,
        { type: 'edge', id: i }, [{ type: 'vertex', id: vEnd }]);
    }
...
function validateHalfEdges(
  ...
  for (let i = 0; i < count; i++) {
    ...
    if (isNullId(edge)) {
      addIssue(report, 'nullReference', 'error', `Half-edge ${i} has null edge reference`, { type: 'halfEdge', id: i });
    }
    
    if (isNullId(loop)) {
      addIssue(report, 'nullReference', 'warning', `Half-edge ${i} has null loop reference`, { type: 'halfEdge', id: i });
    }
    
    if (!isNullId(next)) {
      const nextPrev = model.getHalfEdgePrev(next);
      if (nextPrev !== i) {
        addIssue(report, 'brokenLoopCycle', 'error',
          `Half-edge ${i} -> next ${next} -> prev ${nextPrev} != ${i}`,
          { type: 'halfEdge', id: i }, [{ type: 'halfEdge', id: next }]);
      }
    }
    ...
    if (!isNullId(prev)) {
      ...
      if (prevNext !== i) {
        addIssue(report, 'brokenLoopCycle', 'error',
          ...
          { type: 'halfEdge', id: i }, [{ type: 'halfEdge', id: prev }]);
      }
    }
    ...
    if (!isNullId(twin)) {
      ...
      if (twinTwin !== i) {
        addIssue(report, 'twinMismatch', 'error',
          `Half-edge ${i} -> twin ${twin} -> twin ${twinTwin} != ${i}`,
          { type: 'halfEdge', id: i }, [{ type: 'halfEdge', id: twin }]);
      }
      
      const twinEdge = model.getHalfEdgeEdge(twin);
      if (twinEdge !== edge) {
        addIssue(report, 'halfEdgePairMismatch', 'error',
      ...
      if (dir === twinDir) {
        addIssue(report, 'twinDirectionMismatch', 'error',
    ...
    if (!isNullId(prev) && !isNullId(edge)) {
      ...
      if (prevEnd !== thisStart) {
        addIssue(report, 'vertexMismatch', 'error',
...
function validateLoops(
  ...
  for (let i = 0; i < count; i++) {
    ...
    if (isNullId(face)) {
      addIssue(report, 'nullReference', 'warning', `Loop ${i} has null face reference`, { type: 'loop', id: i });
    } else {
      const faceLoops = model.getFaceLoops(face);
      if (!faceLoops.includes(id)) {
        addIssue(report, 'loopFaceMismatch', 'error',
          `Loop ${i} claims face ${face}, but face doesn't list this loop`,
          { type: 'loop', id: i }, [{ type: 'face', id: face }]);
      }
    }
    
    if (isNullId(firstHe)) {
      addIssue(report, 'nullReference', 'error', `Loop ${i} has null first half-edge`, { type: 'loop', id: i });
      continue;
    }
    ...
    while (actualCount < opts.maxLoopIterations) {
      if (isNullId(current)) {
        addIssue(report, 'loopNotClosed', 'error',
      ...
      if (visited.has(current)) {
        if (current === firstHe) break;
          ...
          addIssue(report, 'brokenLoopCycle', 'error',
            ...
            { type: 'loop', id: i }, [{ type: 'halfEdge', id: current }]);
          break;
        }
      }
    ...
    if (actualCount >= opts.maxLoopIterations) {
      ...
      addIssue(report, 'halfEdgePairMismatch', 'warning',
...
function validateFaces(
  ...
  for (let i = 0; i < count; i++) {
    ...
    if (surfaceIndex >= stats.surfaces || surfaceIndex < 0) {
      addIssue(report, 'surfaceMissing', 'error',
    ...
    if (isNullId(shell)) {
      addIssue(report, 'nullReference', 'warning', `Face ${i} has null shell reference`, { type: 'face', id: i });
    } else {
      const shellFaces = model.getShellFaces(shell);
      if (!shellFaces.includes(id)) {
        addIssue(report, 'faceShellMismatch', 'error',
          `Face ${i} claims shell ${shell}, but shell doesn't list this face`,
          { type: 'face', id: i }, [{ type: 'shell', id: shell }]);
      }
    }
      ...
      for (const loopId of loops) {
        ...
        if (loopFace !== i) {
          addIssue(report, 'loopFaceMismatch', 'error',
            ...
            { type: 'face', id: i }, [{ type: 'loop', id: loopId }]);
        }
      }
    }
  }
...
function validateShells(
  ...
  for (let i = 0; i < count; i++) {
    ...
    if (isNullId(body)) {
      addIssue(report, 'nullReference', 'warning', `Shell ${i} has null body reference`, { type: 'shell', id: i });
    } else {
      const bodyShells = model.getBodyShells(body);
      if (!bodyShells.includes(id)) {
        addIssue(report, 'shellBodyMismatch', 'error',
          `Shell ${i} claims body ${body}, but body doesn't list this shell`,
          { type: 'shell', id: i }, [{ type: 'body', id: body }]);
      }
    }
    
    if (faces.length === 0) {
      addIssue(report, 'nullReference', 'warning', `Shell ${i} has no faces`, { type: 'shell', id: i });
    } else {
      for (const faceId of faces) {
        ...
        if (faceShell !== i) {
          addIssue(report, 'faceShellMismatch', 'error',
            ...
            { type: 'shell', id: i }, [{ type: 'face', id: faceId }]);
        }
      }
    }
  }
...
function validateBodies(
  ...
  for (let i = 0; i < count; i++) {
    ...
    if (shells.length === 0) {
      addIssue(report, 'nullReference', 'warning', `Body ${i} has no shells`, { type: 'body', id: i });
    } else {
      for (const shellId of shells) {
        ...
        if (shellBody !== i) {
          addIssue(report, 'shellBodyMismatch', 'error',
            ...
            { type: 'body', id: i }, [{ type: 'shell', id: shellId }]);
        }
      }
    }
  }
...
function validateManifold(
  ...
  for (const [edgeId, count] of edgeHalfEdgeCounts) {
    if (count < 2 && opts.checkBoundary) {
      addIssue(report, 'boundaryEdge', 'info',
        `Edge ${edgeId} is a boundary edge (only ${count} half-edge(s))`,
        { type: 'edge', id: edgeId });
    } else if (count > 2) {
      addIssue(report, 'nonManifoldEdge', 'error',
  ...
  for (let i = 0; i < stats.shells; i++) {
    ...
    if (model.isShellClosed(id)) {
      ...
      for (const [edgeId, count] of shellEdges) {
        if (count !== 2) {
          addIssue(report, count < 2 ? 'boundaryEdge' : 'nonManifoldEdge', 'error',
            `Closed shell ${i} has edge ${edgeId} with ${count} half-edge(s) (expected 2)`,
            { type: 'shell', id: i }, [{ type: 'edge', id: edgeId }]);
        }
      }
    }
  }
...
function validateSliverFaces(
  ...
  for (let i = 0; i < count; i++) {
    ...
    if (area < model.ctx.tol.length * model.ctx.tol.length) {
      addIssue(report, 'zeroAreaFace', 'warning',
    ...
    if (perimeter > 0) {
      ...
      if (isoperimetricRatio < opts.sliverAspectRatioThreshold) {
        addIssue(report, 'sliverFace', 'warning',
