...
function intersectCollinearSegments2D(
  ...
  if (isZero(d1LenSq, ctx)) {
    ...
    if (
      eqLength(dist2(line1.p0, line2.p0), 0, ctx) ||
 ...
 * @returns Array of intersection points
 */
export function intersectArcArc2D(arc1: Arc2D, arc2: Arc2D, ctx: NumericContext): Intersection2D[] {
...
function isPointOnArc(arc: Arc2D, angle: number, ctx: NumericContext): boolean {
  ...
  if (arc.ccw) {
    if (startNorm <= endNorm) {
      return (
        normalizedAngle >= startNorm - ctx.tol.angle && normalizedAngle <= endNorm + ctx.tol.angle
      );
    } else {
      // Arc crosses 0
      return (
