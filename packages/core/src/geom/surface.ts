...
export function evalSurface(surface: Surface, u: number, v: number): Vec3 {
  switch (surface.kind) {
    case `plane`: {
      return add3(surface.origin, add3(mul3(surface.xDir, u), mul3(surface.yDir, v)));
    }
    ...
    case `cylinder`: {
      ...
      const radial = add3(mul3(basis.vPerp, cosV), mul3(basis.uPerp, sinV));
      return add3(surface.center, add3(mul3(surface.axis, u), mul3(radial, surface.radius)));
    }
    ...
    case `cone`: {
      ...
      return add3(surface.apex, add3(mul3(surface.axis, u), mul3(radial, radiusAtU)));
    }
...
export function surfaceNormal(surface: Surface, u: number, v: number): Vec3 {
  switch (surface.kind) {
    ...
    case `cylinder`: {
      ...
      return normalize3(add3(mul3(basis.vPerp, cosV), mul3(basis.uPerp, sinV)));
    }
    ...
    case `cone`: {
      ...
      const sinV = Math.sin(v);
      // Normal points radially outward (same convention as evalSurface)
      const radial = normalize3(add3(mul3(basis.vPerp, cosV), mul3(basis.uPerp, sinV)));
    ...
    case `sphere`: {
      ...
      return normalize3(vec3(sinU * cosV, sinU * sinV, cosU));
    }
