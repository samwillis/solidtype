...
export function orient3DRobust(a: Vec3, b: Vec3, c: Vec3, d: Vec3): number {
  ...
  return -robustOrient3d(a[0], a[1], a[2], b[0], b[1], b[2], c[0], c[1], c[2], d[0], d[1], d[2]);
}
 ...
 */
export function distanceToPlane(point: Vec3, planeOrigin: Vec3, planeNormal: Vec3): number {
  const toPoint = sub3(point, planeOrigin);
...
export function segSegHit(
  a1: Vec2,
  a2: Vec2, // First segment
  b1: Vec2,
  b2: Vec2 // Second segment
): SegSegResult {
  ...
  if (o1 * o2 < 0 && o3 * o4 < 0) {
    ...
    const point: Vec2 = [a1[0] + t1 * dx1, a1[1] + t1 * dy1];
