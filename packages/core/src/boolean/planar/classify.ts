...
export function classifyPiece(
  ...
  if (result === `outside`) {
    const otherBounds = faceIndex
      ? combineBounds(faceIndex)
      : combineBounds(buildFaceIndex(otherBody, model, ctx));
...
export function _findPointOnFaceMaterial(piece: FacePiece): Vec2 {
  ...
  for (let i = 0; i < n; i++) {
    ...
    if (len > 1e-10) {
      const offset = 0.001;
      const testPt: Vec2 = [
        mid[0] + (towardCenter[0] / len) * offset,
        mid[1] + (towardCenter[1] / len) * offset,
      ];
...
function pickInteriorPoint(piece: FacePiece): Vec2 {
  ...
  for (let i = 1; i < piece.polygon.length - 1; i++) {
    const tri = [piece.polygon[0], piece.polygon[i], piece.polygon[i + 1]] as const;
