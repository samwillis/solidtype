...
export function createHorizontalArcPCurveOnCylinder(
  ...
  const p0_2d = [arc.center[0] + arc.radius * Math.cos(arc.startAngle),
                 arc.center[1] + arc.radius * Math.sin(arc.startAngle)] as Vec2;
  const p1_2d = [arc.center[0] + arc.radius * Math.cos(arc.endAngle),
                 arc.center[1] + arc.radius * Math.sin(arc.endAngle)] as Vec2;
  ...
  void p0_2d; void p1_2d; // Suppress unused warnings
 ...
 */
export function createLine3DCurve(
  model: TopoModel,
  p0: Vec3,
  p1: Vec3
): Curve3DIndex {
