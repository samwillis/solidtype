...
export function extrude(
  ...
  for (let loopIdx = 0; loopIdx < profile.loops.length; loopIdx++) {
    ...
    const bottomFace = createBottomFace(
      model,
      shell,
      profile,
      vertexData,
  ...
  if (namingStrategy && featureId !== undefined) {
    ...
    bottomCapRefs = bottomCapFaces.map((faceId, loopIdx) => {
      ...
      return namingStrategy.recordBirth(
        featureId,
        extrudeBottomCapSelector(loopIdx),
        ref,
        fingerprint
      );
    });
    ...
    sideEdgeRefs = sideEdges.map((loopEdges, loopIdx) =>
      loopEdges.map((edgeId, vertIdx) => {
        ...
        return namingStrategy.recordBirth(
          featureId,
          extrudeSideEdgeSelector(loopIdx, vertIdx),
          ref,
          fingerprint
        );
      })
    ...
    topEdgeRefs = topEdges.map((loopEdges, loopIdx) =>
      loopEdges.map((edgeId, segIdx) => {
        ...
        return namingStrategy.recordBirth(
          featureId,
          extrudeTopEdgeSelector(loopIdx, segIdx),
          ref,
          fingerprint
        );
      })
    ...
    bottomEdgeRefs = bottomEdges.map((loopEdges, loopIdx) =>
      loopEdges.map((edgeId, segIdx) => {
        ...
        return namingStrategy.recordBirth(
          featureId,
          extrudeBottomEdgeSelector(loopIdx, segIdx),
          ref,
          fingerprint
        );
      })
...
function createExtrudeEdges(model: TopoModel, vertexData: ExtrudeVertexData): ExtrudeEdgeData {
...
function createSideFaces(
  ...
  for (let i = 0; i < n; i++) {
    ...
    const edge2D: Vec2 = [endPoint2D[0] - startPoint2D[0], endPoint2D[1] - startPoint2D[1]];
    ...
    if (curve.kind === `arc`) {
      ...
      surface = model.addSurface(
        createPlaneSurface(vec3(pos[0], pos[1], pos[2]), faceNormal, normalize3(edgeDir3D))
      );
    }
