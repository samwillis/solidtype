...
function createTriangleSketch(): { sketch: SketchModel; constraints: Constraint[] } {
  ...
  // Create constraints
  const constraints: Constraint[] = [horizontalLine(l0), equalLength(l1, l2), distance(p0, p1, 10)];
 ...
 */
function createComplexSketch(numPoints: number): {
  sketch: SketchModel;
  ...
  constraints.push(distance(points[0], points[1], 2 * radius * Math.sin(Math.PI / numPoints)));
...
const isMain = typeof process !== `undefined` && process.argv[1]?.endsWith(`solver.bench.ts`);
if (isMain) {
  runSolverBenchmarks();
}
