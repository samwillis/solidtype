...
import { createDocumentMutation, getBranchesForProject, getFoldersForBranch } from '../../lib/server-functions';
import { useLiveQuery } from '@tanstack/react-db';
import { projectsCollection, branchesCollection, foldersCollection } from '../../lib/electric-collections';
import { z } from 'zod';
  ...
  const form = useForm({
    ...
    onSubmit: async ({ value }) => {
      ...
      try {
        ...
        form.setFieldValue('folderId', preselectedFolderId !== undefined ? preselectedFolderId : null);
                ...
                <form.Field
                  ...
                  children={(field) => (
                    <div className="create-dialog-field">
                      ...
                      <input
                        type="text"
                        value={allFolders?.find((f) => f.id === preselectedFolderId)?.name || 'Loading...'}
                        className="create-dialog-input"
                        disabled
                      />
                ...
                children={(field) => (
                  <div className="create-dialog-field">
                    ...
                    >
                      ...
                      >
                        {documentTypes.find((t) => t.value === field.state.value)?.label || 'Select type...'}
                        <LuChevronDown size={12} />
                      </Select.Trigger>
                      <Select.Portal>
                        <Select.Positioner className="create-dialog-select-positioner">
