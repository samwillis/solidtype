...
export function SelectionProvider({ children }: { children: React.ReactNode }) {
  ...
  const [onFaceSelected, setOnFaceSelectedState] = useState<((face: SelectedFace) => void) | undefined>(undefined);
    ...
    return {
      ...
      selectFace: (face: SelectedFace, multi = false) => {
        ...
        setSelectedFaces((prev) => {
          if (multi) {
            // Toggle selection
            const exists = prev.some(f => f.bodyId === face.bodyId && f.faceIndex === face.faceIndex);
            if (exists) {
              return prev.filter(f => !(f.bodyId === face.bodyId && f.faceIndex === face.faceIndex));
            }
            return [...prev, face];
          }
          return [face];
      ...
      selectEdge: (edge: SelectedEdge, multi = false) => {
        setSelectedEdges((prev) => {
          if (multi) {
            const exists = prev.some(e => e.bodyId === edge.bodyId && e.edgeIndex === edge.edgeIndex);
            if (exists) {
              return prev.filter(e => !(e.bodyId === edge.bodyId && e.edgeIndex === edge.edgeIndex));
            }
            return [...prev, edge];
          }
          return [edge];
