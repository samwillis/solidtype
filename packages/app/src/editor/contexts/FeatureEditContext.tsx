...
export function FeatureEditProvider({ children }: FeatureEditProviderProps) {
  ...
  const startExtrudeEdit = useCallback(
    (sketchId: string) => {
  ...
  const startRevolveEdit = useCallback(
    (sketchId: string) => {
  ...
  return <FeatureEditContext.Provider value={value}>{children}</FeatureEditContext.Provider>;
}
